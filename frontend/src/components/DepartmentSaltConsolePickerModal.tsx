  ...
  return createPortal(
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-zinc-950/60 p-4 backdrop-blur-[2px]" onClick={onClose}>
      <div className="flex max-h-[88vh] w-full max-w-3xl flex-col overflow-hidden rounded-[28px] border border-zinc-200 bg-white shadow-[0_32px_80px_rgba(15,23,42,0.28)]" role="dialog" aria-modal="true" aria-labelledby="department-salt-console-picker-title" onClick={(event) => event.stopPropagation()}>
        <div className="border-b border-zinc-200 bg-[radial-gradient(circle_at_top_right,_rgba(56,189,248,0.16),_transparent_28%),radial-gradient(circle_at_left,_rgba(16,185,129,0.12),_transparent_24%),linear-gradient(135deg,_#f8fcff_0%,_#ffffff_58%,_#f6fbf7_100%)] px-6 py-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="inline-flex items-center rounded-full border border-sky-200 bg-sky-50 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.22em] text-sky-700">Department Salt Console</div>
              <h2 id="department-salt-console-picker-title" className="mt-3 text-2xl font-black tracking-tight text-zinc-950">{title}</h2>
              <p className="mt-1 text-sm text-zinc-600">Choose the system that should open the Salt console for this department view.</p>
              <div className="mt-3 inline-flex items-center rounded-full border border-zinc-200 bg-white px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-zinc-600">
                {devices.length} device{devices.length === 1 ? '' : 's'} available
              </div>
            </div>
            <button type="button" onClick={onClose} className="inline-flex items-center rounded-2xl border border-zinc-200 bg-white px-3.5 py-2.5 text-sm font-bold text-zinc-700 shadow-sm transition hover:bg-zinc-50">
          ...
          {devices.length === 0 ? (
            ...
            <div className="grid gap-3">
              ...
              <button
                ...
                className="flex items-center justify-between gap-4 rounded-2xl border border-zinc-200 bg-[linear-gradient(180deg,_#ffffff_0%,_#f9fbff_100%)] px-4 py-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-sky-300 hover:bg-sky-50/50 disabled:opacity-60"
              >
                <div className="min-w-0">
                  <div className="truncate text-sm font-bold text-zinc-950">{device.hostname}</div>
                  <div className="mt-1 text-xs text-zinc-500">{device.osName || 'Unknown OS'}{device.user?.fullName ? ` • ${device.user.fullName}` : ''}</div>
                  <div className="mt-1 text-xs text-zinc-500">{device.department?.name || 'Unassigned department'}</div>
                  {device.minionId ? (
