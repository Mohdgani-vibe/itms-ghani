  ...
  return (
    <>
      {(selectedAlertSource === 'openscap' || selectedAlertSource === 'wazuh' || selectedAlertSource === 'clamav') ? (
        <div className="mt-4 rounded-2xl border border-zinc-200 bg-zinc-50/80 px-4 py-3 text-sm leading-6 text-zinc-600">
          {sourceSummary}
        </div>
      ) : null}

      <div className="mt-4 flex flex-wrap gap-2.5">
        {selectedAlert.assetId ? <button type="button" onClick={onOpenAsset} className="rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm font-bold text-zinc-700 hover:bg-zinc-50">Open Asset</button> : null}
      ...
      {isAlertClosed ? <div className="mt-4 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-700">This alert is resolved. Response actions are read-only for closed findings.</div> : null}
      ...
      {selectedAlertSource === 'openscap' && canResolve && !isAlertClosed && selectedAlert.assetId ? (
        <div className="mt-4 grid gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-4 sm:grid-cols-[minmax(0,220px)_minmax(0,1fr)]">
          <div>
            ...
            <p className="mt-2 text-xs text-zinc-500">{getSaltActionOption(selectedSaltAction).description}</p>
          </div>
          {getSaltActionOption(selectedSaltAction).inputLabel ? (
            <div>
              <label className="mb-1 block text-xs font-bold uppercase tracking-[0.18em] text-zinc-500">{getSaltActionOption(selectedSaltAction).inputLabel}</label>
              <input
                type="text"
          ...
          ) : <div className="text-sm text-zinc-500">Run the selected Salt action against this asset from the alert panel.</div>}
