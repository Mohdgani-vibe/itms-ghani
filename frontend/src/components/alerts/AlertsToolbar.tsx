...
export function AlertsToolbar({
  ...
  return (
    <>
      <div className="border-b border-sky-100 bg-white/80 px-5 py-3 backdrop-blur">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-semibold text-sky-700 shadow-sm transition hover:bg-sky-50"
              >
                {tab.value !== 'all' ? renderSourceIcon(tab.value, 'h-3.5 w-3.5') : <ShieldAlert className="h-3.5 w-3.5" />}
                {tab.label}
                <span className="inline-flex min-w-6 items-center justify-center rounded-full bg-sky-50 px-2 py-0.5 text-xs font-bold text-zinc-800">
                  {count}
                </span>
              </button>
            );
      ...
      {showSearchControls ? (
        <div className="border-b border-sky-100 bg-sky-50/50 px-5 py-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            <div className="relative min-w-0 flex-1">
              ...
              <input
                ...
                className="w-full rounded-xl border border-sky-100 bg-white py-3 pl-10 pr-4 text-sm text-zinc-900 shadow-sm"
              />
            </div>
            ...
            <select
              ...
              className="rounded-xl border border-sky-100 bg-white px-4 py-3 text-sm text-zinc-900 shadow-sm md:w-64"
            >
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-zinc-500">
            <span className="rounded-full bg-white px-2.5 py-1 font-bold text-zinc-700 shadow-sm ring-1 ring-sky-100">
              {totalAlerts} result{totalAlerts === 1 ? '' : 's'}
            </span>
            {sourceFilter !== 'all' ? (
              <span className="rounded-full bg-sky-100 px-2.5 py-1 font-bold text-sky-800">
