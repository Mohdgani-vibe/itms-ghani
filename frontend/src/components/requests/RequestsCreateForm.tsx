...
export default function RequestsCreateForm({
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-[24px] border border-sky-100 bg-[linear-gradient(180deg,_#ffffff_0%,_#f3f9ff_100%)] p-5 shadow-sm">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        ...
        <button type="submit" disabled={requestSubmitting} className="rounded-xl bg-sky-900 px-4 py-2.5 text-sm font-bold text-white hover:bg-sky-800 disabled:opacity-60">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <div>
          ...
          <select value={requestForm.type} onChange={(event) => onTypeChange(event.target.value)} className="w-full rounded-xl border border-sky-100 bg-white px-3 py-3 text-sm text-zinc-900 shadow-sm">
        ...
        <div>
          ...
          <input value={requestForm.title} onChange={(event) => onTitleChange(event.target.value)} className="w-full rounded-xl border border-sky-100 bg-white px-3 py-3 text-sm text-zinc-900 shadow-sm" placeholder="Short request title" />
        </div>
      </div>
      <div className="mt-4">
        <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-zinc-900">Description</label>
        <textarea value={requestForm.description} onChange={(event) => onDescriptionChange(event.target.value)} rows={4} className="w-full rounded-xl border border-sky-100 bg-white px-3 py-3 text-sm text-zinc-900 shadow-sm" placeholder="Describe the issue, needed change, affected employee, or portal setting to update" />
      </div>
    </form>
  );
}