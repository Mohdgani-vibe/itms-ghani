...
export default function RequestQueueTableRow({
  ...
  return (
    <tr className={rowClassName}>
      ...
      <td className="px-4 py-4 align-top">
        <button type="button" onClick={onInspect} className={`text-left ${isEnrollmentRequest ? 'rounded-xl border border-emerald-200 bg-white/90 p-3 shadow-sm' : 'rounded-xl border border-emerald-100 bg-white/90 p-3 shadow-sm hover:bg-emerald-50/60'}`}>
          <div className="flex flex-wrap items-center gap-2">
            {isEnrollmentRequest ? (
              <span className="inline-flex rounded-full bg-emerald-100 px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider text-emerald-700">
                Enrollment Review
              </span>
            ) : null}
          ...
          {isEnrollmentRequest ? (
            <div className="mt-2 space-y-1 text-xs text-emerald-800">
              ...
              <div><span className="font-bold uppercase tracking-wider text-emerald-700">Owner</span> {enrollmentOwner}</div>
              <div><span className="font-bold uppercase tracking-wider text-emerald-700">Department</span> {enrollmentDepartment}</div>
            </div>
          ) : null}
      ...
      <td className="px-4 py-4">
        <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
          ...
          {isEnrollmentRequest ? (
            <>
              <button type="button" onClick={onStart} disabled={!canStart} className={`rounded-lg px-3 py-2 text-xs font-bold transition disabled:opacity-60 ${actionButtonStyles.add}`}>Start</button>
              <button type="button" onClick={onApproveAndOpen} disabled={!canApproveAndOpen || !hasLinkedDevice} className={`rounded-lg px-3 py-2 text-xs font-bold transition disabled:opacity-60 ${actionButtonStyles.save}`}>{hasLinkedDevice ? 'Approve' : 'Await Device Sync'}</button>
            </>
          ) : (
            <button type="button" onClick={onStart} disabled={!canStart} className={`rounded-lg px-3 py-2 text-xs font-bold transition disabled:opacity-60 ${actionButtonStyles.add}`}>Start</button>
          )}
