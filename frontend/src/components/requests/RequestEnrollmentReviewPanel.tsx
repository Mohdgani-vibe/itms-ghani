...
export default function RequestEnrollmentReviewPanel({
  ...
  return (
    <div className="rounded-2xl border border-sky-100 bg-[linear-gradient(180deg,_#ffffff_0%,_rgba(240,249,255,0.9)_100%)] p-4 shadow-sm">
      <div className="text-xs font-bold uppercase tracking-wider text-sky-700">Enrollment Review</div>
      ...
      <div className="mt-3 rounded-xl border border-sky-100 bg-white/90 px-3 py-3">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button type="button" onClick={onOpenDevice} disabled={!canOpenDevice} className="rounded-lg border border-sky-200 bg-white px-3 py-2 text-xs font-bold text-sky-700 hover:bg-sky-50 disabled:cursor-not-allowed disabled:opacity-60">{openDeviceLabel}</button>
        ...
        {showApproveAndOpenAction ? <button type="button" onClick={onApproveAndOpen} disabled={!canApproveAndOpen} className="rounded-lg border border-sky-300 bg-sky-700 px-3 py-2 text-xs font-bold text-white hover:bg-sky-800 disabled:cursor-not-allowed disabled:opacity-60">{approveAndOpenLabel}</button> : null}
