        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`inline-flex items-center rounded-full px-3 py-1.5 text-sm font-semibold transition ${activeTab === tab.key ? 'bg-emerald-100 text-emerald-900 shadow-sm' : 'bg-white text-emerald-700 hover:bg-emerald-50'}`}
            >
            ...
            <div className="rounded-[18px] border border-zinc-200 bg-white p-3 shadow-sm">
              <div className="grid gap-3 lg:grid-cols-[minmax(0,1.2fr)_repeat(3,minmax(0,1fr))]">
                <label className="relative block">
                  ...
                  <input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Search asset tag, serial, item, branch" className="w-full rounded-xl border border-zinc-200 bg-white py-2.5 pl-9 pr-3 text-sm text-zinc-900 outline-none ring-0 placeholder:text-zinc-400 focus:border-zinc-400" />
                </label>
                <select value={mainItemFilter} onChange={(event) => { setMainItemFilter(event.target.value); setSubItemFilter('all'); }} className="rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm text-zinc-700">
                  <option value="all">All main items</option>
                  {items.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
                </select>
                <select value={subItemFilter} onChange={(event) => setSubItemFilter(event.target.value)} className="rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm text-zinc-700">
                ...
                <select value={assetTypeFilter} onChange={(event) => setAssetTypeFilter(event.target.value)} className="rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm text-zinc-700">
                  ...
                  <tbody className="divide-y divide-zinc-100">
                    {assets.map((asset) => (
                      <tr key={asset.id} className="hover:bg-zinc-50/80">
                        ...
                        <td className="px-4 py-3 text-zinc-600">{asset.itemCode}</td>
                        <td className="px-4 py-3 text-zinc-600">{asset.branchName || 'Unassigned'}</td>
                    ...
                    {!loading && assets.length === 0 ? (
                      <tr>
                        <td colSpan={7} className="px-4 py-8 text-center text-sm text-zinc-500">No assets found for the current filters.</td>
                      </tr>
                    ) : null}
                  </tbody>
                </table>
